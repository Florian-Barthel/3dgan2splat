...
class GaussianModel:
    ...
    def training_setup(self, training_args, decoder_params=None, sr_params=None):
        ...
        if decoder_params is not None:
            l.append({'params': decoder_params, 'lr': training_args.rotation_lr, "name": "decoder"})

        if sr_params is not None:
            l.append({"params": sr_params, "lr": 0.0001, "name": "superresolution", "weight_decay": 1e-4})
    ...
    def replace_tensor_to_optimizer(self, tensor, name):
        ...
        for group in self.optimizer.param_groups:
            if group["name"] in ["decoder", "superresolution"]:
                continue
            if group["name"] == name:
