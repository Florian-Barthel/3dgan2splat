...
class LookAtPoseSampler:
    ...
    @staticmethod
    def sample(horizontal_mean, vertical_mean, lookat_position, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cuda'):
        h = torch.randn((batch_size, 1), device=device) * horizontal_stddev + horizontal_mean
        v = torch.randn((batch_size, 1), device=device) * vertical_stddev + vertical_mean
        v = torch.clamp(v, 1e-5, math.pi - 1e-5)
...
class UniformCameraPoseSampler:
    ...
    @staticmethod
    def sample(horizontal_mean=3.14 / 2, vertical_mean=3.14 / 2, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu'):
...
def FOV_to_intrinsics(fov_degrees, imsize=1.414, device='cpu'):
    """
...
def FOV_to_intrinsics_orig(fov_degrees, device='cpu'):
    """
    Creates a 3x3 camera intrinsics matrix from the camera field of view, specified in degrees.
    Note the intrinsics are returned as normalized by image size, rather than in pixel units.
    Assumes principal point is at image center.
    ...
    fov_rad = fov_degrees * 2 * 3.14159 / 360
    focal_length = float(1 / (math.tan(fov_rad / 2) * 1.414))
    intrinsics = torch.tensor([
        [focal_length,  0,              0.5],
        [0,             focal_length,   0.5],
        [0,             0,              1.0]
