...
class TriPlaneGenerator(torch.nn.Module):
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        ...
        # return {'image': None, 'image_raw': torch.zeros([1, 3, 128, 128], device="cuda"), 'image_depth': None, "planes": planes}
        ...
        #return {'image': None, 'image_raw': rgb_image, 'image_depth': depth_image, "planes": planes}
...
class OSGDecoder(torch.nn.Module):
    ...
    def forward(self, sampled_features, ray_directions):
        ...
        N, M, C = x.shape
        x = x.view(N*M, C)
        x_copy = x.prod(-1, keepdim=True)[None, ...]
        x = self.net(x)
        x = x.view(N, M, -1)
        rgb = torch.sigmoid(x[..., 1:])*(1 + 2*0.001) - 0.001 # Uses sigmoid clamping from MipNeRF
        sigma =  x[..., 0:1] # torch.where(x_copy == 0, -10, 10) # *  x[..., 0:1]
